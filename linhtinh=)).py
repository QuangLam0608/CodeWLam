...
def check_collision(pipes):
    for pipe in pipes:
        if bird_rect.colliderect(pipe): #chạm vào ống
            hit_sound.play()
            return False
    if  bird_rect.bottom >= 650: #vượt quá cửa sổ pygame or chạm sàn
            return False
    return True 
...
#chèn background
bg = pygame.image.load('assets/bgparis.png').convert() #convert giúp đổi file hình thành một file nhẹ hơn giúp load lẹ hơn
...
#tạo chim
bird_down = pygame.transform.scale2x(pygame.image.load('assets/peodown.png').convert_alpha())
bird_mid = pygame.transform.scale2x(pygame.image.load('assets/peomid.png').convert_alpha())
bird_up = pygame.transform.scale2x(pygame.image.load('assets/peoup.png').convert_alpha())
